...
void main() {
  group('BluetoothState', () {
    ...
    test('should provide correct connection state descriptions', () {
      expect(BluetoothState(connectionState: BluetoothConnectionState.disconnected).connectionStateDescription, '未连接');
      expect(BluetoothState(connectionState: BluetoothConnectionState.connecting).connectionStateDescription, '连接中...');
      expect(BluetoothState(connectionState: BluetoothConnectionState.connected).connectionStateDescription, '已连接');
      expect(BluetoothState(connectionState: BluetoothConnectionState.disconnecting).connectionStateDescription, '断开连接中...');
      expect(BluetoothState(connectionState: BluetoothConnectionState.error).connectionStateDescription, '连接错误');
    });

    test('should provide correct scan state descriptions', () {
      expect(BluetoothState(scanState: BluetoothScanState.idle).scanStateDescription, '空闲');
      expect(BluetoothState(scanState: BluetoothScanState.scanning).scanStateDescription, '扫描中...');
      expect(BluetoothState(scanState: BluetoothScanState.stopped).scanStateDescription, '已停止');
      expect(BluetoothState(scanState: BluetoothScanState.error).scanStateDescription, '扫描错误');
    });

    test('should provide correct status flags', () {
      final connectingState = BluetoothState(connectionState: BluetoothConnectionState.connecting);
      final connectedState = BluetoothState(connectionState: BluetoothConnectionState.connected);
      final scanningState = BluetoothState(scanState: BluetoothScanState.scanning);
    ...
    test('should create string representation correctly', () {
      ...
      expect(state.toString(), 'BluetoothState(connection: 已连接, scan: 空闲, devices: 2)');
    });
  });
}
